...
angular.module('iguanaApp.controllers').controller('walletHomeController', 
  ...
  this.gen_btc_addr = function() {
    $rootScope.account.generateNEWaddress();
...
/*
  ...
  this.copyAddress = function(addr) {
    if (isCordova) {
      ...
      window.plugins.toast.showShortCenter(getString('Copied to clipboard'));
        ...
        "_customAlternative", {
          ...
          set: function(newValue) {
            ...
            if (typeof(newValue) === 'number' && $scope.isRateAvailable) {
              $scope.customAmount = null;//parseFloat((rateService.fromFiat(newValue, $scope.alternativeIsoCode) * satToUnit).toFixed($scope.unitDecimals), 10);
            } else {
      ...
      Object.defineProperty($scope,
        "_customAmount", {
          ...
          set: function(newValue) {
            ...
            if (typeof(newValue) === 'number' && $scope.isRateAvailable) {
              $scope.customAlternative = null;//parseFloat((rateService.toFiat(newValue * $scope.unitToSatoshi, $scope.alternativeIsoCode)).toFixed(2), 10);
            } else {
      ...
      "_alternative", {
        ...
        set: function(newValue) {
          ...
          if (typeof(newValue) === 'number' && self.isRateAvailable) {
            $scope._amount = null;//parseFloat((rateService.fromFiat(newValue, self.alternativeIsoCode) * satToUnit).toFixed(self.unitDecimals), 10);
          } else {
            $scope.__amount = null;
          }
        },
    ...
    Object.defineProperty($scope,
      "_amount", {
        ...
        set: function(newValue) {
          ...
          if (typeof(newValue) === 'number' && self.isRateAvailable) {
            $scope.__alternative = null;//parseFloat((rateService.toFiat(newValue * self.unitToSatoshi, self.alternativeIsoCode)).toFixed(2), 10);
          } else {
  ...
  this.setSendError = function(err) {
    var fc = profileService.focusedClient;
    var prefix =
      fc.credentials.m > 1 ? getString('Could not create payment proposal') : getString('Could not send payment');
...
/*
    ...
    self.setOngoingProcess(getString('Creating transaction'));
    $timeout(function() {
  ...
  this._setOngoingForSigning = function() {
    ...
    if (fc.isPrivKeyExternal() && fc.getPrivKeyExternalSourceName() == 'ledger') {
      self.setOngoingProcess(getString('Requesting Ledger Wallet to sign'));
    } else {
      self.setOngoingProcess(getString('Signing payment'));
    }
  };
  ...
  this.signAndBroadcast = function(txp, cb) {
    ...
    profileService.signTxProposal(txp, function(err, signedTx) {
      ...
      if (err) {
        ...
        err.message = bwsError.msg(err, getString('The payment was created but could not be signed. Please try again from home screen'));
        return cb(err);
      }

      if (signedTx.status == 'accepted') {
        self.setOngoingProcess(getString('Broadcasting transaction'));
        fc.broadcastTxProposal(signedTx, function(err, btx, memo) {
          self.setOngoingProcess();
          if (err) {
            err.message = bwsError.msg(err,getString('The payment was signed but could not be broadcasted. Please try again from home screen'));
            return cb(err);
          }
          if (memo)
            $log.info(memo);
  ...
  this.setFromPayPro = function(uri, cb) {
    ...
    self.setOngoingProcess(getString('Fetching Payment Information'));
...
*/
  ...
  this.sendAll = function() {
    var self = this;
    self.error = null;
    self.setOngoingProcess(getString('Calculating fee'));
    $rootScope.$emit('Local/SetFeeSendMax', function(currentFeePerKb, availableMaxBalance, feeToSendMaxStr) {
      ...
      var msg = getString("{{fee}} will be deducted for bitcoin networking fees", {
        fee: feeToSendMaxStr
      });
/*
      $scope.$apply();
